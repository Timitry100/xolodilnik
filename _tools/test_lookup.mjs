// Тест серверного поиска товара по штрихкоду (личная база + OFF + парсинг)
...
for (const code of codes) {
  try {
    ...
    console.log(code, '→', p ? `${p.name} (источник: ${p._source})${p.brand ? ' / ' + p.brand : ''}${p.volume ? ' / ' + p.volume : ''}` : 'не найден');
  } catch (e) {
